...
async def get_sources_from_items(
    ...
    # AUTO-KNOWLEDGE: if no items, inject all accessible knowledge bases
    if not items and user:
        try:
            import open_webui.models.knowledge as _kb_mod
            all_kbs = _kb_mod.Knowledges.get_knowledge_bases()
            items = [
                {"type": "collection", "id": kb.id}
                for kb in all_kbs
                if user.role == "admin" or kb.user_id == user.id
            ]
        except Exception as e:
            log.debug(f"AUTO-KNOWLEDGE injection failed: {e}")
